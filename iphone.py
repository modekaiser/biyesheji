...
# 创建文件对象（追加模式），首次创建才写表头
csv_path = 'shuju/all.csv'
file_exists = os.path.exists(csv_path) and os.path.getsize(csv_path) > 0
f = open(csv_path, mode='a', newline='', encoding='UTF-8-sig') #a 追加模式
...
# 打开浏览器
qci = ChromiumPage()
#苹果17 苹果17promax https://item.jd.com/100209268193.html
qci.get('https://item.jd.com/100209268193.html')
...
for page in range(1, 101):
    print(f'正在采集iPhone第{page}页的数据内容')
    if page <= 2:
